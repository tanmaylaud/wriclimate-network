...
def edge_labels_creation(Graph, pos):
    ...
    for edge in Graph.edges():
        ...
        x1 = coor1[0]
        y1 = coor1[1]
        midpoint_coor = midpoint(x0, y0, x1, y1)
    ...
    #links = list(nx.get_edge_attributes(Graph, 'link').values())
    #relations = []
    #for i in range(0,len(labels)):
    #    relations.append("<a href={link} >{label}</a>".format(link=links[i],label=labels[i]))
    edge_trace_text = go.Scatter(x=mid_point_x, y=mid_point_y, 
                           mode='markers+text',
                           text=labels,
                           textfont=dict(color="crimson"),
                           marker={'symbol': 'circle', 'size': 15, 'color': "lightpink", 'line_width': 0},
                           )
...
def arrow_trace_creation(Graph, pos):
    ...
    for edge in Graph.edges():
        ...
        if not (x0 == x1 and y0 == y1):
            arrow_trace_x, arrow_trace_y = addEdge(coor0, coor1, arrow_trace_x, arrow_trace_y, 1, 'end', .02, 20, 10)
    arrow_trace = go.Scatter(x=arrow_trace_x, y=arrow_trace_y, line=dict(width=1, color="black"), hoverinfo='none', mode='lines')
    return arrow_trace
...
def get_graph_fig(G):
    ...
    fig = full_graph_creation([node_trace, edge_trace, edge_labels, arrow_trace])
    return fig